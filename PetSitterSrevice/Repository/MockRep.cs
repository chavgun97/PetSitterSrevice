...
namespace PetSitterSrevice.Repository
{
    public class MockRep : IRep
    {
        ...
        public int CreateOrder(Order_x order)
        {
            ...
            orders.Add(order);
            users.First(x => x.Id == order.User.Id).Orders.Add(order);
        ...
        public int CreatePet(Pet_x pet)
        {
            ...
            users.First(x => x.Id == pet.User.Id).Pets.Add(pet);
        ...
        public User_x GetUserByToken(string UID)
        {
        ...
        public int UpdateOrder(Order_x order)
        {
            ...
            var userOrders = users.First(x => x.Id == order.User.Id).Orders;
            userOrders.Remove(userOrders.First(x => x.Id == order.Id));
            userOrders.Add(order);

            var sitterOrders = sitters.First(x => x.Id == order.User.Id).Orders;
        ...
        {
            var userPets = users.First(x => pet.User.Id == x.Id).Pets;
        ...
        public int UpdateUser(User_x user)
        {
            ...
            orders.Where(x => x.User.Id == x.User.Id).ToList().ForEach(x =>x.User = user);
